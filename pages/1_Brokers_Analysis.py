...
def load_and_normalize_data(mongodb_uri, database_name):
    ...
    # Check for the existence of 'Date' in brokerage_data
    if 'Date' in brokerage_data.columns:
        brokerage_data['Date'] = pd.to_datetime(brokerage_data['Date'], errors='coerce')
    else:
        st.error("Brokerage data is missing the 'Date' column. Please check your data source.")
        # Return an empty DataFrame for brokerage_data so that the app can handle it gracefully.
        return listings_data, pd.DataFrame()
...
def main():
    ...
    filtered_brokerage = brokerage_data[(brokerage_data['Date'] >= pd.Timestamp(start_date)) &
    ...
    fig_market_share.update_traces(mode="lines+markers")
    ...
    top_brokers = merged_monthly['Broker'].unique()
    filtered_firm_data_top = monthly_firm_data[monthly_firm_data['Firm'].isin(top_brokers)]
    ...
    # Mapping of agents to their respective firms
