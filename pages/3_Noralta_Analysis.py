...
#####################################################################################
    ...
    with col2:
        ...
        # Re-filter noralta_agents_data based on the applied filters
        noralta_agents_data = noralta_data[
            (noralta_data['Listing Firm 1 - Office Name'] == 'Royal LePage Noralta Real Estate') |
            (noralta_data['Buyer Firm 1 - Office Name'] == 'Royal LePage Noralta Real Estate')
        ]

        # Apply filters to noralta_agents_data
        if selected_agents:
            noralta_agents_data = noralta_agents_data[
                (noralta_agents_data['Listing Agent 1 - Agent Name'].isin(selected_agents)) |
                (noralta_agents_data['Buyer Agent 1 - Agent Name'].isin(selected_agents))
            ]
        if selected_cities:
            noralta_agents_data = noralta_agents_data[noralta_agents_data['Area/City'].isin(selected_cities)]
        if selected_communities:
            noralta_agents_data = noralta_agents_data[noralta_agents_data['Community'].isin(selected_communities)]
        if selected_property_types:
            noralta_agents_data = noralta_agents_data[noralta_agents_data['Property Class'].isin(selected_property_types)]
        if selected_building_types:
            noralta_agents_data = noralta_agents_data[noralta_agents_data['Building Type'].isin(selected_building_types)]
        if selected_years:
            noralta_agents_data = noralta_agents_data[noralta_agents_data['Year Built'].isin(selected_years)]

        # Check if there are any Noralta agents after filtering
        if noralta_agents_data.empty:
            st.warning("No Noralta agents found for the selected filters.")
        else:
